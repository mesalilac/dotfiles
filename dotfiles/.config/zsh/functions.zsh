...
yt-select() {
    ...
    yt-dlp -o "%(title)s.%(ext)s" --windows-filenames --embed-thumbnail --add-metadata -ic -f $format_id $URL
}

yt-audio() {
    URL=$1
    [ -z $URL ] && echo -e "You must provide a URL." && return 1
    yt-dlp -o "%(title)s.%(ext)s" --windows-filenames --audio-quality 0 --embed-thumbnail --add-metadata -ic --extract-audio -t mp3 "$URL"
}

yt() {
    URL=$1
    [ -z $URL ] && echo -e "You must provide a URL." && return 1
    yt-dlp -o "%(title)s.%(ext)s" --windows-filenames --embed-thumbnail --add-metadata -ic -f "bestvideo[ext=mp4]+bestaudio[ext=m4a]/best[ext=mp4]/best" $URL
}
